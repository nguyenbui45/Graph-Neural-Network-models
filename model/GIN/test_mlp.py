...
class GIN(nn.Module):
    def __init__(self,
        ...
        self.mlps = torch.nn.ModuleList() # declare list of MLP
        self.batch_norms = torch.nn.ModuleList() # declate list of batch norm
        ...
        for layer in range(self.num_layers - 1):
            ...
            self.batch_norms.append(nn.BatchNorm(hidden_dim))
    ...
    def __preprocess_neighbor_sumavepool(self,batch_graph):
        ...
        if not self.learn_eps: # if not distinguish the center node & its neighbors,  add selfloop to the center node
            ...
            Adj_block_index = torch.concat([Adj_block_element,element],0)
    ...
    def next_layer_esp(self,h,layer,padded_neighbor_list=None,Adj_block=None):
    ...
    def forward(self,batch_graph):
        ...
        if self.neighbor_pooling_type == 'max':
            pooled_neighbor_list = self.__preprocess_neighbor_maxpool(batch_graph)

        else:
            Adj_block = self.__process_neighbor_sumavepool(batch_graph)

        hidden_rep = [X_concat]
        h = x_concat 
        
        for layer in range(self.num_layers - 1):
            if self.neighbor_pool_type == 'max' and self.learn_eps:
                h = self.next_layer_eps(h,layer, padded_neighbor_list = padded_neighbor_list)
            elif self.neighbor_pool_type == 'max' and not self.learn_eps:
                h =self.next_layer(h,layer,padded_neighbor_list = padded_neighbor_list)
            elif not self.neighbor_pool_type == 'max' and self.learn_eps:
                h = self.next_layer_eps(h,layer,padded_neighbor_list = Adj_block)
            elif not self.neighbor_pool_type == 'max' and not self.learn_eps:
        ...
        for layer,h in enumerate(hidden_rep):
            pooled_h = torch.spmm(graphpool,h)
